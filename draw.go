...
func drawStep(i_step uint64, worker_c chan bool, x_start float64, x_end float64,
    ...
    if err != nil { panic("err in open file for reading") }
    defer file.Close()
    ...
    for _, body := range bodies_json {
        if body.X >= x_end || body.Y >= y_end { continue }
        if body.X < x_start || body.Y < y_start { continue }
        x := int(float64(w) * (body.X - x_start) / x_end)
        y := int(float64(h) * (body.Y - y_start) / y_end)
